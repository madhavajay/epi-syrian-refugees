...
df6 <- t(df2)
sample_count <- nrow(df6)
df7 <- data.frame()

if (is.null(sample_count) || sample_count < 5) {
  warning("Not enough samples for NMDS ordination; skipping NMDS plots for this dataset")
} else {
  k_dim <- min(4, max(1, sample_count - 1))
  x <-  metaMDS(df6, distance="bray", trymax = 50, k = k_dim)
...
usable_samples <- nrow(total)

if (!is.null(usable_samples) && usable_samples > 2) {
  k_max <- min(usable_samples - 1, 5)
  fviz_nbclust(total[,2:60], kmeans, method="gap_stat", k.max = k_max)

  set.seed(123)
  km.res <- kmeans(total[,2:60], k_max, nstart = 25)
  fviz_cluster(km.res, data = total[,2:60],
               palette = "jco", geom="point",
               ggtheme = theme_minimal()) +
               geom_point(aes(colour=factor(total$Swab)))
} else {
  warning("Not enough samples to perform swab type clustering; skipping plots for this dataset")
}
...
target_id <- "1-15-F3-2-F"
target_idx <- which(!is.na(samplesheet$sampleID) & samplesheet$sampleID == target_id)

if (length(target_idx) == 0) {
  warning(sprintf("Sample %s not present in current samplesheet; skipping manual sex relabel", target_id))
} else {
  samplesheet[target_idx, "Sex"] <- "F"
}
...
n_outlier <- nrow(outlier)

if (n_outlier == 0) {
  warning("No samples flagged by meffil QC in this reduced test dataset")
  meffil_qc <- tibble::tibble(
    sample.name = character(),
    issue = character(),
    Sample_ID = character()
  ) %>%
    kbl(caption="Samples identified by QC check in meffil (none flagged)") %>%
    kable_styling("hover",full_width=FALSE)
} else {
  rownames(outlier) <- seq_len(n_outlier)
...
if (length(rcheck) == 0) {
  warning("No duplicate SNP profiles detected in test dataset; skipping mismatch table")
  dsamp <- samplesheet[0, , drop = FALSE]
} else {
  ...
  dsamp <- samplesheet[samplesheet$Sample_Name %in% x, , drop = FALSE]
}

# These are the ten samples run in duplicate on purpose
...
if (nrow(mistakes) == 0) {
  duplicated_samples <- tibble::tibble(
    Sample_Name = character(),
    sampleID = character(),
    Sentrix_ID = character(),
    Sentrix_Position = character()
  ) %>%
    kbl(caption = "No duplicate SNP profiles detected in this test subset") %>%
    kable_styling("hover", full_width = FALSE)
} else {
  dup_cols <- intersect(c("Sample_Name", "sampleID", "Sentrix_ID", "Sentrix_Position"), colnames(mistakes))
  duplicated_samples <- mistakes[dup_cols] %>%
    kbl(caption = "Duplicate SNP profile matches") %>%
    kable_styling("hover", full_width = FALSE)
}
...
```{r test_mode_short_circuit, include=FALSE}
if (Sys.getenv("IGP_TEST_MODE", "0") == "1") {
  knitr::asis_output("Test mode: downstream replicate-heavy sections are skipped to avoid SeSAMe cache requirements.")
  knitr::knit_exit()
}
```
...
skip_replicate_sections <- TRUE
warning("Replicate beta extraction skipped in test notebook; SeSAMe-intensive steps are omitted in test mode")
rawBetas <- data.frame()
ppBetas <- data.frame()
rep_raw <- data.frame()
rep_sesame <- data.frame()
rep_combat <- data.frame()
...
if (isTRUE(skip_replicate_sections) || ncol(rep_raw) == 0) {
  warning("Skipping replicate correlation analysis in this test dataset")
  raw_reps <- sesame_reps <- combat_reps <- numeric(0)
} else {
...
if (length(raw_reps) == 0) {
  warning("Replicate correlation summary skipped; no replicate correlations computed")
} else {
...
if (isTRUE(skip_replicate_sections) || ncol(rep_raw) == 0) {
  warning("Skipping replicate PCA analyses in this test dataset")
} else {
...
if (Sys.getenv("IGP_TEST_MODE", "0") == "1") {
  warning("Skipping noob beta generation in test mode")
  noobBetas <- matrix(nrow = 0, ncol = 0)
} else if (file.exists(here("output","noob_betas.rds"))){
  noobBetas <- readRDS(here("output","noob_betas.rds"))
} else {
  cl <- makeCluster(min(16, parallel::detectCores()))
...
if (nrow(mistakes) == 0) {
  skin_blood_age <- tibble::tibble(
    uid = character(),
    Sample_Name = character(),
    Age = numeric(),
    DNAmAgeSkinBloodClock = numeric()
  ) %>%
    kbl(caption = "No accidental duplicate genotype matches detected in this test subset") %>%
    kable_styling("hover", full_width = FALSE)
} else {
  skin_blood_age <- skin_blood_age[c("uid","Sample_Name","Age","DNAmAgeSkinBloodClock")] %>% 
