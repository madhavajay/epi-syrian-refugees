...
```{r setup_stage_control, include=FALSE}
# Stage-based execution control
# Set IGP_STAGE environment variable to control execution:
#   1 = Setup & Initial QC (up to label technical replicates)
#   2 = Read IDATs & Correlations (up to cor table) [DEFAULT]
#   3 = Replicate Analysis (up to use horvath age data)
#   4 = Full QC (up to combat batch correction)
#   5 = Complete Analysis (all chunks)

IGP_STAGE <- as.integer(Sys.getenv("IGP_STAGE", "2"))
message(paste("Running up to STAGE", IGP_STAGE))

# Stage breakpoints (chunk numbers from grep output)
STAGE_CHUNKS <- list(
  stage1 = 640,   # "label technical replicates"
  stage2 = 1421,  # "cor table"
  stage3 = 1799,  # "use horvath age data"
  stage4 = 2461,  # "combat batch correction"
  stage5 = 9999   # All chunks
)

# Helper function to check if current chunk should run
should_run_chunk <- function(chunk_line) {
  if (IGP_STAGE == 5) return(TRUE)
  max_line <- STAGE_CHUNKS[[paste0("stage", IGP_STAGE)]]
  return(chunk_line <= max_line)
}
```
...
# Debug: Check mistakes dataframe
cat("\n=== DEBUG: Mistakes dataframe ===\n")
cat(paste("Number of mistake samples:", nrow(mistakes), "\n"))
cat(paste("Number of columns in mistakes:", ncol(mistakes), "\n"))
if (nrow(mistakes) > 0) {
    cat("Column names:", paste(colnames(mistakes), collapse=", "), "\n")
    cat("First few rows:\n")
    print(head(mistakes))
} else {
    cat("No accidental duplicate samples found\n")
}
cat("==================================\n\n")
...
if (n_mistakes > 0) {
    # Safely select columns - only select column 7 if it exists
    max_col <- ncol(mistakes)
    if (max_col >= 7) {
        mistake_cols <- mistakes[c(1:3,7)]
    } else {
        # If column 7 doesn't exist, just use columns 1:3
        mistake_cols <- mistakes[c(1:min(3, max_col))]
    }

    duplicated_samples <- mistake_cols %>%
      kbl(caption=paste("Duplicated samples:", n_mistakes, "found")) %>%
      kable_styling("hover",full_width=F)

    # Apply row styling based on actual number of mistakes
    # IMPORTANT: n_mistakes is the original row count, but we need to verify
    # the actual row count in the rendered table
    if (n_mistakes >= 2) {
...
# Debug: Write to file for real-time viewing
debug_file <- "output/toAverage_debug.txt"
dir.create("output", showWarnings = FALSE, recursive = TRUE)
sink(debug_file)
cat("\n=== DEBUG: Creating toAverage for replicate analysis ===\n")
cat(paste("Timestamp:", Sys.time(), "\n"))
cat(paste("Total samples in samplesheet:", nrow(samplesheet), "\n"))

# Check which samples have replicates
replicate_counts <- samplesheet %>%
  group_by(sampleID) %>%
  summarise(n_replicates = n()) %>%
  filter(n_replicates > 1)

cat(paste("Samples with replicates (n>1):", nrow(replicate_counts), "\n"))
if (nrow(replicate_counts) > 0) {
  cat("Replicate sample IDs:\n")
  print(replicate_counts)
} else {
  cat("⚠ WARNING: No samples with multiple replicates found!\n")
}
...
cat(paste("\nAfter filtering, toAverage has", nrow(toAverage), "rows\n"))
if (nrow(toAverage) > 0) {
  cat("First few rows of toAverage dataframe:\n")
  print(head(toAverage, 10))
} else {
  cat("⚠ WARNING: toAverage is empty after filtering!\n")
}

cat("\nCalling by() to group uid by sampleID...\n")
toAverage <-by(toAverage$uid,toAverage$sampleID,print)

cat(paste("After by(), toAverage class:", class(toAverage), "\n"))
cat(paste("After by(), toAverage length:", length(toAverage), "\n"))
if (length(toAverage) > 0) {
  cat(paste("Names:", paste(names(toAverage), collapse=", "), "\n"))
  cat("\nFirst element of toAverage:\n")
  print(toAverage[[1]])
} else {
  cat("⚠ WARNING: toAverage has length 0 after by()!\n")
}
cat("=======================================================\n\n")
sink()

message("✓ toAverage debug written to ", debug_file)


```

```{r stage1_complete, include=FALSE}
# STAGE 1 CHECKPOINT: Setup & Initial QC Complete
if (IGP_STAGE == 1) {
  message("\n========================================")
  message("STAGE 1 COMPLETE: Setup & Initial QC")
  message("========================================")
  message("✓ Libraries loaded")
  message("✓ Data loaded")
  message("✓ SNP clustering done")
  message("✓ NMDS done")
  message("✓ Meffil QC steps 1-5 complete")
  message("✓ Technical replicates labeled")
  message("✓ toAverage structure created")
  message("\nDebug output: output/toAverage_debug.txt")
  message("========================================\n")
  knitr::knit_exit()
}
```
...
cat("\n")
cat("================================================================================\n")
cat("CHUNK: readIdats_reps - Starting IDAT reading and preprocessing\n")
cat("================================================================================\n")
cat(paste("Timestamp:", Sys.time(), "\n"))
cat(paste("Working directory:", getwd(), "\n"))
cat(paste("Samples to process:", nrow(samplesheet), "\n"))
cat("\n")

# Ensure ALL required sesame data is cached (required for IDAT reading)
cat("[STEP 1/10] Checking sesame reference data cache...\n")
options(timeout = 600)  # Increase timeout for slow connections

# Show ExperimentHub cache location
eh_cache <- rappdirs::user_cache_dir("ExperimentHub")
cat(paste("  ExperimentHub cache directory:", eh_cache, "\n"))

# Try caching ALL sesame data (most reliable approach despite being slower)
cat("\n[STEP 2/10] Attempting to cache all sesame data (may take several minutes)...\n")
cat("  This will download reference files from ExperimentHub\n")
cat("  If this hangs, it's likely a network/ExperimentHub server issue\n\n")

cache_start <- Sys.time()
tryCatch({
  sesameDataCache()
  cache_elapsed <- as.numeric(difftime(Sys.time(), cache_start, units = "secs"))
  cat(paste("  ✓ All sesame data cached successfully in", round(cache_elapsed, 1), "seconds\n"))
}, error = function(e) {
  cat(paste("  ✗ Bulk cache failed:", e$message, "\n"))
  cat("\n[STEP 2b/10] Falling back to individual file caching...\n")

  # Fallback: cache specific files
  ...
  for (file in required_files) {
    tryCatch({
      ...
      cat(paste("✓ (", round(file_elapsed, 1), "s)\n", sep = ""))
    }, error = function(e2) {
      cat(paste("✗ FAILED -", e2$message, "\n"))
    })
  }
})

cat("\n[STEP 3/10] Sesame cache setup complete\n\n")

# Get raw betas first

# Limit nested threading before any heavy processing so cached paths benefit too
Sys.setenv(
...
if (file.exists(here("output","raw_betas.rds"))){
  ...
  cat("[STEP 5/10] Preparing to read IDAT files...\n")
  ...
  cat("  Setting locale to C for multibyte string handling... ")
  ...
  cat(paste("\n  File existence check for first sample:\n"))
  cat(paste("    ", basename(first_sample), "\n"))
  cat(paste("      Grn:", file.exists(grn_file), "| Grn.gz:", file.exists(grn_gz), "\n"))
  cat(paste("      Red:", file.exists(red_file), "| Red.gz:", file.exists(red_gz), "\n"))

  # Try to read first sample as a test
  cat("\n[STEP 6/10] Running test read on first sample...\n")
  cat("  This validates sesame can read the IDAT files\n")
  ...
  cat("\n[STEP 7/10] Setting up parallel processing for raw betas...\n")
  # Use IGP_CORES from environment, or auto-detect with sensible max
  igp_cores <- as.integer(Sys.getenv("IGP_CORES", "8"))
  n_cores <- min(igp_cores, parallel::detectCores() - 1, 16)
  cat(paste("  Creating cluster with", n_cores, "workers (IGP_CORES=", igp_cores, ")... ", sep=""))
  cl <- makeCluster(n_cores)
  registerDoParallel(cl)
  cat("✓\n")

  # Load required packages in each worker - THIS IS CRITICAL!
  cat("  Loading packages in each worker (sesame, here)... ")
  clusterEvalQ(cl, {
    library(sesame)
    library(here)
    Sys.setlocale("LC_ALL", "C")
  ...
  cat(paste("\n  Starting parallel IDAT reading for", nrow(samplesheet), "samples...\n"))
  cat("  This may take 10-30 minutes depending on system\n")
  read_start <- Sys.time()
  ...
  read_elapsed <- as.numeric(difftime(Sys.time(), read_start, units = "mins"))
  cat(paste("\n  Parallel reading completed in", round(read_elapsed, 1), "minutes\n"))
  ...
  cat(paste("    Successful:", length(successful_reads), "| Failed:", length(errors), "\n"))

  # Report errors if any
  if (length(errors) > 0) {
    err_msg <- paste("\n=== Raw IDAT Read Errors (", length(errors), "samples failed) ===\n")
    cat(err_msg)
    cat(err_msg, file = here("output", "debug_trace.txt"), append = TRUE)

    for (i in 1:min(5, length(errors))) {
      err <- errors[[i]]
      err_detail <- paste("  Sample:", basename(err$pfx), "- Error:", err$error, "\n")
      cat(err_detail)
      cat(err_detail, file = here("output", "debug_trace.txt"), append = TRUE)
    }
    if (length(errors) > 5) {
      more_msg <- paste("  ... and", length(errors) - 5, "more\n")
      cat(more_msg)
      cat(more_msg, file = here("output", "debug_trace.txt"), append = TRUE)
    }
    cat("=====================================\n\n")
    cat("=====================================\n", file = here("output", "debug_trace.txt"), append = TRUE)
  }

  if (length(successful_reads) == 0) {
    cat("\n✗ All raw IDAT reads failed!\n")
    ...
    cat("FATAL: All raw IDAT reads failed\n", file = here("output", "debug_trace.txt"), append = TRUE)
    stop("All raw IDAT reads failed. Check output/debug_trace.txt for details.")
  }

  cat(paste("\n✓ Successfully read", length(successful_reads), "of", nrow(samplesheet), "samples\n"))

  cat("  Combining results into matrix... ")
  rawBetas <- do.call(cbind, successful_reads)
  cat(paste("✓ (", nrow(rawBetas), "probes ×", ncol(rawBetas), "samples)\n"))

  cat("  Saving to output/raw_betas.rds... ")
...
cat("[STEP 8/10] Processing preprocessed betas (with noob, pOOBAH, etc.)...\n")

# Ensure nested threading stays at 1 even when raw betas were cached earlier
Sys.setenv(
  RCPP_PARALLEL_NUM_THREADS = "1",
  OMP_NUM_THREADS = "1",
  OMP_THREAD_LIMIT = "1",
...
if (file.exists(here("output","preprocessed_betas.rds"))){
  cat("  ✓ Found cached preprocessed_betas.rds - loading from disk\n")
  ppBetas <- readRDS(here("output","preprocessed_betas.rds"))
  cat(paste("  Loaded", ncol(ppBetas), "samples,", nrow(ppBetas), "probes\n\n"))

} else {
  cat("  No cache found - will preprocess IDATs from scratch\n")
  cat("  This applies: dyeBiasNL → inferInfiniumIChannel → pOOBAH → noob\n\n")
  ...
  cat(paste("\n  Starting parallel preprocessing for", nrow(samplesheet), "samples...\n"))
  cat("  This will take longer than raw reads (30-60 minutes)\n")
  ...
  preprocess_elapsed <- as.numeric(difftime(Sys.time(), preprocess_start, units = "mins"))
  cat(paste("\n  Parallel preprocessing completed in", round(preprocess_elapsed, 1), "minutes\n"))
  ...
  if (length(successful_reads) == 0) {
    cat("FATAL: All preprocessed IDAT reads failed\n", file = here("output", "debug_trace.txt"), append = TRUE)
    stop("All preprocessed IDAT reads failed. Check output/debug_trace.txt for details.")
  }

  cat(paste("\n✓ Successfully preprocessed", length(successful_reads), "of", nrow(samplesheet), "samples\n"))

  cat("  Combining results into matrix... ")
  ppBetas <- do.call(cbind, successful_reads)
  cat(paste("✓ (", nrow(ppBetas), "probes ×", ncol(ppBetas), "samples)\n"))

  cat("  Saving to output/preprocessed_betas.rds... ")
...
cat("[STEP 9/10] Preparing data for replicate analysis...\n")
# Now get combat betas just for replicates

cat("  Converting to data frames and setting column names... ")
...
cat("  Subsetting replicate samples... ")
...
cat(paste("✓ (", ncol(rep_raw), "replicates)\n\n"))


cat("[STEP 10/10] Running ComBat batch correction on replicates...\n")
if (file.exists(here("output","rep_combat.rds"))){
  cat("  ✓ Found cached rep_combat.rds - loading from disk\n")
  rep_combat <- readRDS(here("output","rep_combat.rds"))
  cat(paste("  Loaded", ncol(rep_combat), "samples\n\n"))
} else {
  cat("  No cache found - running ComBat from scratch\n")

  # prepare sesame preprocess replicate betas for ComBat.
  cat("  Preparing data for ComBat... ")
...
# ComBat will not run on probes where everyone is NA. Eliminate
  ...
  # Check if we have multiple batches for ComBat
  n_batches <- length(unique(batch$Plate))
  cat(paste("  Detected", n_batches, "unique batch(es) in replicate samples\n"))

  if (n_batches < 2) {
    cat("  ⚠ Warning: Only 1 batch found - skipping ComBat (requires ≥2 batches)\n")
    cat("  Using preprocessed betas without batch correction\n")
    # rep_combat already contains the preprocessed data, no need to modify
  } else {
    # Convert betas to m values prior to combat
    cat("  Converting beta values to M values... ")
    mvals <- as.matrix(BetaValueToMValue(rep_combat))
    modcombat <- model.matrix(~1,data=batch)
    cat("✓\n")

    # Run ComBat with a parametric Bayesian framework
    cat(paste("  Running ComBat batch correction across", n_batches, "batches (this may take a few minutes)...\n"))
    ...
    mvals <- ComBat(
      ...
      mean.only = FALSE,
      ref.batch = NULL)
    combat_elapsed <- as.numeric(difftime(Sys.time(), combat_start, units = "secs"))
    cat(paste("  ✓ ComBat completed in", round(combat_elapsed, 1), "seconds\n"))

    # convert mvals object into a data frame of beta values
    cat("  Converting M values back to beta values... ")
  ...
  cat("  Saving to output/rep_combat.rds... ")
...
cat("\n")
cat("================================================================================\n")
cat("CHUNK COMPLETE: readIdats_reps\n")
cat("================================================================================\n")
cat(paste("Completion timestamp:", Sys.time(), "\n"))
cat(paste("Raw betas:", ncol(rawBetas), "samples ×", nrow(rawBetas), "probes\n"))
cat(paste("Preprocessed betas:", ncol(ppBetas), "samples ×", nrow(ppBetas), "probes\n"))
cat(paste("Replicate combat betas:", ncol(rep_combat), "samples\n"))
cat("================================================================================\n")
cat("\n")
...
# Debug: Check what's in toAverage - write to file
debug_cor_file <- "output/correlation_debug.txt"
dir.create("output", showWarnings = FALSE, recursive = TRUE)
sink(debug_cor_file, append=TRUE)
cat("\n=== DEBUG: toAverage structure for correlations ===\n")
cat(paste("Timestamp:", Sys.time(), "\n"))
cat(paste("Class:", class(toAverage), "\n"))
cat(paste("Length:", length(toAverage), "\n"))
cat(paste("Is list:", is.list(toAverage), "\n"))
cat(paste("Names:", paste(names(toAverage), collapse=", "), "\n"))
cat("\nFirst few elements:\n")
print(head(toAverage, 3))
cat("===================================\n\n")
sink()

message("✓ toAverage structure debug written to ", debug_cor_file)

# Check if we have replicate data to correlate
if (length(toAverage) == 0 || !is.list(toAverage)) {
  cat("⚠ Warning: No replicate samples found or toAverage is not properly structured\n")
  cat("Skipping correlation matrix analysis\n")
  # Create empty placeholder objects so downstream code doesn't break
  raw_reps <- numeric(0)
  sesame_reps <- numeric(0)
  combat_reps <- numeric(0)
} else {
  cat(paste("Calculating correlations for", length(toAverage), "replicate groups\n"))

  # Check if expected replicate names exist
  expected_names <- c("1-11-F3-2-F", "1-13-F3-2-M", "2-6-F3-1-F", "2-10-F2-F",
                      "2-10-F3-2-F", "3-5-F3-2-F", "3-6-F3-1-M", "3-15-F3-1-F",
                      "3-21-F2-F", "3-24-F3-1-F")
  available_names <- names(toAverage)
  missing_names <- setdiff(expected_names, available_names)

  if (length(missing_names) > 0) {
    cat("⚠ WARNING: Expected replicate groups not found in toAverage:\n")
    cat(paste("  Missing:", paste(missing_names, collapse=", "), "\n"))
    cat(paste("  Available:", paste(available_names, collapse=", "), "\n\n"))
  }

# Raw replicate correlations with error handling
cat("Calculating raw correlations...\n")

raw_rep1 <- tryCatch({
  cols <- toAverage$`1-11-F3-2-F`
  cat(paste("  rep1: selecting", length(cols), "columns\n"))
  rep_raw %>% select(all_of(cols)) %>% corstars(method = "spearman")
}, error = function(e) { cat(paste("  rep1 FAILED:", e$message, "\n")); NULL })

raw_rep2 <- tryCatch({
  cols <- toAverage$`1-13-F3-2-M`
  cat(paste("  rep2: selecting", length(cols), "columns\n"))
  rep_raw %>% select(all_of(cols)) %>% corstars(method = "spearman")
}, error = function(e) { cat(paste("  rep2 FAILED:", e$message, "\n")); NULL })

raw_rep3 <- rep_raw %>% 
  select(all_of(toAverage$`2-6-F3-1-F`)) %>% 
  corstars(method = "spearman")

raw_rep4 <- rep_raw %>% 
  select(all_of(toAverage$`2-10-F2-F`)) %>% 
  corstars(method = "spearman")

raw_rep5 <- rep_raw %>% 
  select(all_of(toAverage$`2-10-F3-2-F`)) %>% 
  corstars(method = "spearman")

raw_rep6 <- rep_raw %>% 
  select(all_of(toAverage$`3-5-F3-2-F`)) %>% 
  corstars(method = "spearman")

raw_rep7 <- rep_raw %>% 
  select(all_of(toAverage$`3-6-F3-1-M`)) %>% 
  corstars(method = "spearman")

raw_rep8 <- rep_raw %>% 
  select(all_of(toAverage$`3-15-F3-1-F`)) %>% 
  corstars(method = "spearman")

raw_rep9 <- rep_raw %>% 
  select(all_of(toAverage$`3-21-F2-F`)) %>% 
  corstars(method = "spearman")

raw_rep10 <- rep_raw %>% 
  select(all_of(toAverage$`3-24-F3-1-F`)) %>% 
  corstars(method = "spearman")
...
sesame_rep1 <- rep_sesame %>% 
  select(all_of(toAverage$`1-11-F3-2-F`)) %>% 
  corstars(method = "spearman")

sesame_rep2 <- rep_sesame %>% 
  select(all_of(toAverage$`1-13-F3-2-M`)) %>% 
  corstars(method = "spearman")
...
}  # End of replicate correlation analysis
...
# Check if we have correlation data to display
if (length(raw_reps) == 0) {
  cat("⚠ No correlation data available (skipped due to missing replicates)\n")
  cat("Displaying placeholder table\n")

  # Create placeholder data frame
  scor <- data.frame(
    replicate = "No replicates available",
    raw_betas = NA,
    SeSame_betas = NA,
    ComBat_betas = NA
  )
  ...
  scor %>%
    kbl(escape = F,
        caption="Spearman Correlations for Technical Replicates (No Data Available)") %>%
    kable_styling("hover", full_width = F)
...
```{r stage2_complete, include=FALSE}
# STAGE 2 CHECKPOINT: Read IDATs & Correlations Complete
if (IGP_STAGE == 2) {
  message("\n========================================")
  message("STAGE 2 COMPLETE: Read IDATs & Correlations")
  message("========================================")
  message("✓ All chunks from Stage 1")
  message("✓ IDAT files read (raw betas)")
  message("✓ Preprocessed betas computed")
  message("✓ ComBat batch correction applied")
  message("✓ Correlation matrices calculated")
  message("✓ Correlation table generated")
  message("\nDebug outputs:")
  message("  - output/toAverage_debug.txt")
  message("  - output/correlation_debug.txt")
  message("  - output/raw_betas.rds")
  message("  - output/preprocessed_betas.rds")
  message("  - output/rep_combat.rds")
  message("========================================\n")
  knitr::knit_exit()
}
```
...
targets <- read.metharray.sheet(here("data"), pattern = "cMulligan_SampleSheet160\\.csv$")
