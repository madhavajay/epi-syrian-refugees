...
if (file.exists(here("output","raw_betas.rds"))){
  ...
  # Export locale settings to workers
  clusterEvalQ(cl, {
    Sys.setlocale("LC_ALL", "C")
  })
  ...
  rawBetas <- do.call(cbind, parLapply(cl,samplesheet$Basename, function(pfx) {
    tryCatch({
      ...
      warning(paste("Failed to read IDAT:", pfx, "-", e$message))
      return(NULL)
    })
    }))
  ...
  rawBetas <- rawBetas[, !sapply(rawBetas, is.null), drop = FALSE]
...
if (file.exists(here("output","preprocessed_betas.rds"))){
  ...
  ppBetas <- do.call(cbind, parLapply(cl,samplesheet$Basename, function(pfx) {
    tryCatch({
      ...
      warning(paste("Failed to preprocess IDAT:", pfx, "-", e$message))
      return(NULL)
    })
    }))
  ...
  # Remove any NULL results from failed reads
  ppBetas <- ppBetas[, !sapply(ppBetas, is.null), drop = FALSE]
