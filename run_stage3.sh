...
if [ "$DOCKER_MODE" = true ] && [ "$NO_DOCKER_FLAG" = false ]; then
    ...
    docker run --rm -v "$(pwd)":/workspace -w /workspace -e IGP_CORES="${IGP_CORES:-}" -e IGP_TEST_MODE="$([ "$TEST_MODE" = true ] && echo 1 || echo 0)" \
        epi-syrian-refugees Rscript -e "setwd('/workspace'); system('./run_stage3_rscript.R ${PASSTHRU_ARGS[*]}')"
    exit $?
fi
...
RENDER_CMD="rmarkdown::render('script/igp_epigenetic_age_quality_control_20230611.Rmd')"
